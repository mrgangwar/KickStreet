...
export default function EditProduct() {
  ...
  return (
    <AdminLayout>
      {/* Container: Adjusted radius and padding for mobile-first accessibility */}
      <div className="w-full max-w-4xl mx-auto bg-white p-5 sm:p-10 rounded-[2rem] sm:rounded-[3.5rem] shadow-xl border border-gray-100 mb-10 overflow-hidden">
        <div className="mb-8 sm:mb-10 text-center sm:text-left">
           <h2 className="text-3xl sm:text-5xl font-black italic uppercase mb-2 tracking-tighter text-black leading-none">Edit Product</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 sm:grid sm:grid-cols-2 sm:gap-6">
          ...
          {/* Stock & Sizes */}
          <div className="group">
            <label className="text-[9px] sm:text-[10px] font-black uppercase ml-1 sm:ml-2 mb-2 block text-gray-400">Stock</label>
          ...
          <div className="sm:col-span-2">
            <label className="block text-[9px] sm:text-[10px] font-black uppercase mb-4 text-gray-400 ml-1 sm:ml-2">Product Images</label>
                ...
                <div className="flex gap-4 overflow-x-auto pb-4 w-full scrollbar-hide">
                  {images.map((img, i) => (
                    <div key={i} className="relative flex-shrink-0">
                       <img src={img} alt={`Preview ${i}`} className="w-32 h-32 object-cover rounded-2xl sm:rounded-[2rem] border-2 border-gray-50 shadow-sm" />
                       <button type="button" onClick={()=>setImages(images.filter((_,index)=>index!==i))} 
                         className="absolute -top-1 -right-1 bg-black text-white w-7 h-7 rounded-full font-bold text-sm flex items-center justify-center shadow-lg hover:bg-red-600 transition-colors">×</button>
                    </div>
                  ))}
                </div>
            </div>
